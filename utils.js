...
let generateServerMakeCredRequest = (username, displayName, id) => {
    return {
        ...
        user: {
            id: id,
            name: username, //email in our case
        ...
        //   "excludeCredentials": [
        //     {
        //       "id": "-lNFfiiim6QVkueDp_BrtOzxqTCz90C9Q-6tK6bybD8",
        //       "type": "public-key",
        //       "transports": [
        //         "internal"
        //       ]
 ...
 */
let generateServerGetAssertion = (authenticators, id) => {
    let allowCredentials = [];
    for(let authr of authenticators) {
        if(authr && authr !== null && authr !== undefined){
            console.log(authr, ' test error !' )
...
let verifyAuthenticatorAttestationResponse = (webAuthnResponse) => {
    ...
    if(ctapMakeCredResp.fmt === 'fido-u2f') {
        ...
        console.log(authrDataStruct, ' Auth Data STr!!!!')
        ...
        let authrDataStruct = parseMakeCredAuthData(ctapMakeCredResp.authData);
        console.log(authrDataStruct, ' aut-d')
        if(!(authrDataStruct.flags & U2F_USER_PRESENTED))
            throw new Error('User was NOT presented durring authentication!');
        ...
        // if(response.verified) {
            ...
            console.log(response.authrInfo, ' authrINfo ')
        // }
    }else {
...
let verifyAuthenticatorAssertionResponse = (webAuthnResponse, authenticators) => {
    ...
    let response = {'verified': false};
    console.log(authenticatorData, ' Auth Data for Login! ')
    if(authr.fmt === 'fido-u2f') {
        ...
        console.log(response.verified, ' response awaited!', signatureBase, ' clienthash', clientDataHash)
        if(response.verified) {
